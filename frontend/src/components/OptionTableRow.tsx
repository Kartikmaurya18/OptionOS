...
function OptionTableRowImpl({ strike, style }: OptionTableRowProps) {
  ...
  return (
    <TableRow style={style} className="absolute left-0 top-0 w-full">
      <TableCell className="font-medium text-call">{formatPrice(row?.callLtp)}</TableCell>
      <TableCell className="font-medium text-put">{formatPrice(row?.putLtp)}</TableCell>
      <TableCell className="font-semibold text-strike">{formatStrike(strike)}</TableCell>
      <TableCell className="font-semibold text-straddle">{formatPrice(row?.straddle)}</TableCell>
    </TableRow>
  );
}
